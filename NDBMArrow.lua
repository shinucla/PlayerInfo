...
arrow:SetTexture("Interface\\AddOns\\PlayerInfo\\Arrow.blp")
...
-- -- Original calculateDistance that use MapSizes
...
do
   ...
   function updateArrow(direction, distance)
      ...
      --print("distance: "..tostringall(distance));
      if distance then
...
local function show(runAway, x, y, distance, time)
   ...
   hideDistance = -1;  ---- kz: override hideDistance passed in (never hide)
   hideTime = nil;      ---- kz: override hideTIme passed in (never hide)
...
function NDBMArrow:ShowRunTo(...)
   return show(false, ...)
end

function NDBMArrow:ShowRunAway(...)
   return show(true, ...)
end

function NDBMArrow:Hide(autoHide)
   frame:Hide()
end
...
function NDBMArrow:LoadPosition(point, x, y)
   --local x = 5;
   --local y = -240;
